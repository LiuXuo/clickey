...
; ========================== 说明 ==========================
; 1) 3 层结构：
;    第 1 层：9x9 双键（行键 3x3 + 列键 3x3 组合）
;      行键：wer / sdf / xcv
;      列键：uio / jkl / m,.
;    第 2 层：3x3 单键（wer / sdf / xcv）
;    第 3 层：3x3 单键（uio / jkl / m,.）
; 2) 按键：Ctrl+; 左键 / Ctrl+Shift+; 右键 / Ctrl+Shift+Alt+; 中键
; 3) 交互：Esc 取消；Backspace 回退上一步（无历史则退出）；
;    Space 直接点击当前区域中心点并跳过后续层级
; 4) 额外：Ctrl+Alt+D 显示 DPI/屏幕调试信息
