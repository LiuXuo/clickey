...
> 当前仓库阶段：**Tauri 原型可用 + AHK 行为基准并行**。正式产品继续沿 **Tauri + Rust + Svelte** 方向演进（Windows/macOS/Linux\*）。
...
仓库里有一套版本化的 AutoHotkey（AHK v1）行为原型，集中在 `demo/`：
...
完整迭代说明见 `demo/clickey.md`；这些脚本用于验证交互与算法（撤销/取消/直达点击、多显示器坐标、DPI 缩放、遮罩绘制等）。
...
2. 双击运行 `demo/clickey_v3.1.ahk`（最新基准）；如需对比历史行为，可运行 `demo/clickey_v1.0.ahk` 或 `demo/clickey_v1.1.ahk`。
...
- 激活热键收敛为单一字段：`hotkeys.activation.trigger`（默认 `Ctrl+;`）。
- Overlay 内动作切换键改为可配置：`hotkeys.controls.switchAction`（默认 `Enter`，循环左/右/中）。
- 切换显示器键改为可配置：`hotkeys.controls.nextMonitor`（默认 `Tab`）。
- Settings 页面已支持上述控制键编辑；Overlay 右上角动作提示会显示当前动作与当前切换键。
...
### 当前基准：v3.1（`demo/clickey_v3.1.ahk`）
...
AHK 原型负责回答“交互是否成立、算法是否准确、遮罩是否可用”，后续行为与算法以 `demo/clickey_v3.1.ahk` 为基准：
