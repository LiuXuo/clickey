...
; 3) 单键层：主键 qwert/asdfg/zxcvb，且支持替代键
;    替代键：yuiop/hjkl;/nm,./（与主键映射到同一格）
; 4) 单键层不绘制字母标签（仅显示网格）
...
Clickey_Init() {
    ...
    singleAltRows := []
    singleAltRows.Push(["y","u","i","o","p"])
    singleAltRows.Push(["h","j","k","l",";"])
    singleAltRows.Push(["n","m",",",".","/"])
    ...
    layer2.map := Clickey_BuildDualKeyMap(singleRows, singleAltRows)
...
Clickey_BuildDualKeyMap(rowsPrimary, rowsAlt) {
    map := {}
    idx := 0
    for r, row in rowsPrimary {
        for c, k in row {
            idx += 1
            map[k] := idx
            alt := rowsAlt[r][c]
